...
public class RecipeSuggester {
	...
	// hardcoded since the bot was rewritten from scratch, this was the last tweet replied to years ago
        ...
        while (true) {
            ...
            try {
	            ...
	            for (Status tweet : queryResult.getTweets()) {
	            	...
	            	// do this before we tweet, in case something goes wrong we don't want to try it again
	            	LAST_TWEET_ID_REPLIED_TO = tweet.getId();
	
	            	Recipe recipe = getRecipeSuggested(tweet.getText());
	            	
	            	String tweetToBeSent = "@" + tweet.getUser().getScreenName() +
	                		" Your recipe is " + recipe.getRecipeName() + " " + recipe.getRecipeUrl();
	                /*
	                 * In reality this shouldn't really happen often, but if it does we'll show the url
	                 * before the recipe name, and then make it shorter enough for a tweet
	                 */
	            	if (tweetToBeSent.length() > TWEET_MAX_CHARACTERS) {
	            		tweetToBeSent = "@" + tweet.getUser().getScreenName() + " " + 
	            			recipe.getRecipeUrl() + " Your recipe is " + recipe.getRecipeName();
